...
public class NetworkClient {
  ...
  public NetworkClient(String baseUrl, String webSocketUrl) {
    ...
    webSocketClient = buildStompOverWebSocketClient(webSocketUrl, httpClient);
  }
...
//  private Completable sendUserNotReady(UUID userId) {
//    String url = WS_APP_USER_NOT_READY_DESTINATION.replace("{uuid}", userId.toString());
//    Timber.i("%s send %s to %s", connectedUser.getName(), UserStatus.NOT_READY_TO_PLAY, url);
//    return webSocketClient.send(url, "")
//            .observeOn(Schedulers.io())
//            .subscribeOn(Schedulers.computation());
//  }
  ...
  public void match(ConnectedUser user, final MatchMessageVisitor playerHandler) {
  ...
  Disposable watchUserStatus(ConnectedUser user,
